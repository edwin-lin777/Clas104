  ...
  return (
    <div className="page fade-up" style={{maxWidth:820}}>
      ...
      <div style={{background:"white",border:"1px solid var(--rule)",borderRadius:2,overflow:"hidden"}}>
        <div className="equiv-row" style={{background:"var(--parchment2)",borderBottom:"1px solid var(--rule)",padding:"0.75rem 1.25rem"}}>
          {["Greek","Roman","Domain"].map(h=><div key={h} className={`small-caps ${h==="Domain"?"equiv-domain":""}`} style={{fontSize:9}}>{h}</div>)}
        </div>
        {greekRoman.map((row,i)=>(
          <div key={row.greek} className="equiv-row" style={{padding:"0.75rem 1.25rem",borderBottom:i<greekRoman.length-1?"1px solid var(--rule2)":"none",background:i%2===0?"transparent":"rgba(245,240,232,0.5)"}}>
            ...
            <div className="equiv-domain" style={{fontSize:14,color:"var(--warm-mid)"}}>{row.domain}</div>
          </div>
        ))}
      </div>
    </div>
